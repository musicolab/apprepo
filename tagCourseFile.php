		...
		<div class="row">
			...
			if(isset($_GET["action"]) && isset($_GET['selectedtags'])){
				if($_GET["action"]=='addnewtags'){
					...
					$res = $con->query("select * from files2tags where fileId=".$_GET['fid']);
					...
					if(!$res){
						...
						for($i=0; $i<count($_GET['selectedtags']); $i=$i+1){
							...
							if($sel!="") {
								...
								$con->query("insert into files2tags values(NULL, ".$_GET['fid'].", '".$sel."')");
							}
						}
					}else{
						...
						while($row = mysqli_fetch_assoc($res)){
							...
							if($found==false){ # row[tag] in not in the list of selected, it has to go
								...
								$resDel = $con->query("delete from files2tags where fileId=".$_GET['fid']." and tag='".$row['tag']."'");
								//echo $resDel."<br>";
						...
						for($i=0; $i<count($_GET['selectedtags']); $i=$i+1){
							...
							if($found==false){ #the selected tag is not in the db
								if($sel!="") $con->query("insert into files2tags values(NULL, '".$_GET['fid']."','".$sel."')");
							}
						}
					}
				}
                        ...
                        $fid  = $_GET['fid'];
                        $fname  = $_GET['fname'];
                        $sql  = "select * from metadata where filetype='course' and id=".$fid;
			...
			<div class="col-sm-9">
				<div class="panel panel-default" style='border:1px solid #ddd;'>
                                ...
                                ?>
				<h3><?echo $_GET['cname']?> / Tags / <?echo $_GET['fname']?></h3>
				<hr style='background-color:#ddd'>
				<form action='tagCourseFile.php' method='' id='theform'>
					<?
					...
					<div class='row'>
						<div class='col-sm-5'>
							...
							<select name='existingtags' id='existing' size='10' style='width:100%' multiple readonly>
								<?
								$seltagsres = $con->query("select distinct(tag) from files2tags where tag not in ( select tag from files2tags where fileId=".$_GET['fid'].") ");
								echo "select distinct(tag) from files2tags where fileId!=".$_GET['fid']."<br>";
						...
						<div class='col-sm-5'>
							...
							<select name='selectedtags[]' id='selected' size='10' style='width:100%' multiple readonly>
		                                                <?
		                                                $sql = "select * from files2tags where fileId=".$fid;
		                                                $res = $con->query($sql);
		                                                while($row = mysqli_fetch_assoc($res)){
