...
md`# MusiColab Reposiotry
_Similarity of musical pieces based on tempo, chroma, energy, etc_
...
function _chart(l,n,d3,width,height,color,drag,invalidation) {
  ...
  const gethtml = (id, links) => {
    return `<div><font face="Helvetica" size="3px" color="##FFC300"><b>Audio File</b>:</font> <font face="Helvetica" size="3px" >${id}</font></div> 
            ...
            <audio controls>
              <source src="jazz_1.ogg" type="audio/ogg"> <source src="jazz_1.mp3" type="audio/mpeg"></audio>`;
  }
  /*
  ...
  */
  var simulation = d3.forceSimulation(nodes,links)
    .force("link", d3.forceLink(links).distance(linkDistance(nodes)).id(d => d.id))
...
function _l(FileAttachment){return(
FileAttachment("links@2.json").json()
)}

function _n(FileAttachment){
	return(FileAttachment("network@5.json").json())
}
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ["network@5.json", {url: new URL("./files/network@5.json", import.meta.url), mimeType: "application/json", toString}],
    ["links@2.json", {url: new URL("./files/links@2.json", import.meta.url), mimeType: "application/json", toString}]
  ]);
